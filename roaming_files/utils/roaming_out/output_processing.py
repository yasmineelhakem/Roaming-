...
def country_operator(vlr_number):
    try:
        vlr_number_str = str(vlr_number)
        phone_number = phonenumbers.parse("+" + vlr_number_str)
        country_code = region_code_for_number(phone_number)
        country = pycountry.countries.get(alpha_2=country_code)
        country_name = country.name if country else "Country not found"

        ro_number = phonenumbers.parse("+"+vlr_number_str, country_code)
        operator_name = carrier.name_for_number(ro_number, "en")
